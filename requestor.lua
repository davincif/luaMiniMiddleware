...
--[[
	...
	elseif(proto ~= nil and lsok.is_proto_valid(proto) == false) then
		...
		conf.print("serviço está em: "..ip.."   "..port)
		if(ip == conf.notFound) then
			...
			if(chat.reged == false) then
				if(request.sign("chat", ip, port, proto) == false) then
					...
					chat.skey, bytes = crh.send(strm.service.."("..chat.cname..","..strm.load..")", chat.skey, ip, port, {proto = proto, ip = request.clientIP, port = request.clientPort})
